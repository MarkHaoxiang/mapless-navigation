...
class Scenario(BaseScenario):
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        for wall, pos, orientation in self.walls:
            wall.set_pos(
                torch.tensor(
                    [pos[0], pos[1]], dtype=torch.float32, device=self.world.device
                ),
            ...
            if orientation == Direction.VERTICAL:
                ...
                wall.set_rot(
                    torch.tensor(
                        3 * torch.pi / 2, dtype=torch.float32, device=self.world.device
                    ),
                    batch_index=env_index,
                )
            else:
    ...
    def _random_room_position(self, distance_from_edge: float = 0) -> torch.tensor:
        ...
        assert (
            bounding_box[0] <= bounding_box[1] and bounding_box[2] <= bounding_box[3]
